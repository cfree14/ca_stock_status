...
source("data/kelp_scuba/helper_functions.R")
...
# Export
saveRDS(pred_grid, file=file.path(datadir, "SCUBA_prediction_grid.Rds"))
...
count(data, zone, level) %>% arrange(desc(n))
...
for(i in 1:length(spp_list)){
  ...
  model <- try(sdmTMB(
    data = sdata,
    formula = count ~ -1 + as.factor(year), # yday as smoother created problems
    spatiotemporal = "iid",
    time = "year",
    spatial="on",
    mesh = mesh,
  ...
  if(inherits(model, "try-error")){
    message("Model failed for iteration ", i, ". Skipping...")
    next  
  }
  ...
  # QQ-plot
  qq <- qqnorm(mdata$resid, plot.it = F)
  qq_df <- tibble(x=qq$x,
                  y=qq$y)
